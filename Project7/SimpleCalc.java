	...
	class ButtonListener implements ActionListener
	{
		public void actionPerformed (ActionEvent e)
		{
			...
			for (int i = 0; i < 4; i++)
			{
				...
				if (whichButton == ops[i] && express.length() > 0)
				{
					char last = express.charAt(express.length() - 1);
					if (last == '0' || last == '1' || last == '2' || last == '3' || last == '4' || last == '5'
						|| last == '6' || last == '7' || last == '8' || last == '9')
			
							expression.setText(express + opCodes[i]);
				}
			...
			if (whichButton == equals)
			{
				...
				if (express != null && express != "") 
				{
					...
					|| express.charAt(express.length() - 1) == '*' || express.charAt(express.length() - 1) == '/')
					...
					else
					{
						...
						for (String tokens : operandList)
						{
							double operators = Double.valueOf(tokens);
							operandDouble.add(operators);
						}
